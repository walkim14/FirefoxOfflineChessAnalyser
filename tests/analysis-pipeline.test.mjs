...
import { analyzeGameLine } from "../app/analysis-pipeline.mjs";
...
class MockEngine {
  async analyze(fen, { multiPV = 1 } = {}) {
    ...
    const cpWhite = this.evaluateMaterial(game);

    const lines = [];
    for (let i = 0; i < Math.max(1, multiPV); i += 1) {
      const move = moves[i] || best;
      if (!move) {
        break;
      }
      ...
      lines.push({
        ...
        pv: uci,
        scoreType: "cp",
        scoreValue: side === "w" ? cpWhite - i * 12 : -cpWhite - i * 12,
        cpWhite: cpWhite - i * 12,
        evalText: `${((cpWhite - i * 12) / 100).toFixed(2)}`,
        winPercentWhite: 50,
      });
    }
...
test("analysis pipeline completes on provided chess.com game and uses PGN Elo", async () => {
  const parsed = parsePgnToLine(CHESS_COM_PGN, Chess);
  assert.equal(parsed.whiteElo, 1578);
  assert.equal(parsed.blackElo, 1560);
  assert.equal(parsed.suggestedElo, 1569);

  const engine = new MockEngine();
  const report = await analyzeGameLine({
    startFen: parsed.startFen,
    lineMoves: parsed.lineMoves,
    playerElo: parsed.suggestedElo,
    depth: 12,
    multiPV: 2,
    ChessImpl: Chess,
    engine,
  ...
  assert.equal(report.plies, parsed.lineMoves.length);
  assert.equal(report.results.length, parsed.lineMoves.length);
  assert.ok(report.results.every((entry) => typeof entry.classification.label === "string"));
});
